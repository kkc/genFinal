...
main(int argc, char **argv, char **envp)
{
  ...
  while(result != NULL)
  {
      strcat( path_name, result );
      result = strtok( NULL, delims );
      strcat( path_name, "/");
      if ( strcmp(result,"a.out") == 0) break;
  }
//  printf("\n%s\n",path_name);
  char result_path_name[80];
  strcpy( result_path_name, path_name);
  strcat( path_name, "fragment.log");
  strcat( result_path_name, "solution.log");

  FILE *fp = fopen(path_name,"r");
  create_fragment_table(fp);

  fp_result = fopen(result_path_name,"w");
